...
const ProTipCard: React.FC<ProTipCardProps> = ({ shouldFetch }) => {
    ...
    useEffect(() => {
        if (!isDismissed && shouldFetch) {
            fetchTip();
        }
    }, [fetchTip, isDismissed, shouldFetch]);
    ...
    return (
        <div className="glassmorphism-enhanced tilt-card p-4 rounded-xl shadow-lg h-full flex flex-col">
            <div className="flex justify-between items-center mb-2">
                <h3 className="text-subtitle text-secondary">{t.pro_tip_of_the_day}</h3>
            ...
            {isCardLoading ? (
                ...
                <p className="text-body text-base-content-dark flex-1">💡 {translatedData?.tip}</p>
            )}
        </div>
    );
};
