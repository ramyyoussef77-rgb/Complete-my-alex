...
const ChatMessageBubble: React.FC<ChatMessageBubbleProps> = ({ message, currentUser, room, onRetry }) => {
    ...
    const MessageStatus = () => {
        ...
        if (message.status === 'failed') {
            return <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-red-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>;
        }
        return null;
    };
    ...
    return (
        <div className={`flex items-end gap-2 group ${isUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`relative max-w-xs md:max-w-md p-3 rounded-lg shadow-md ${isUser ? 'bg-gradient-to-r from-primary to-secondary text-white' : 'bg-base-200/80 dark:bg-base-dark-200/80 backdrop-blur-sm'}`}>
            ...
            {message.status === 'failed' && isUser && (
                <button onClick={() => onRetry(message)} className="text-xs text-red-400 hover:underline">{t.retry_send}</button>
            )}
        </div>
    );
};
