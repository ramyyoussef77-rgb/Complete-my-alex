        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4 animate-page-fade-in" onClick={onClose}>
            <div className="bg-base-dark-200 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 p-4 overflow-y-auto">
                    ...
                    <p className="text-white/80 mt-4">{item.description}</p>
                    
                    {item.tags && item.tags.length > 0 && (
                        <div className="mt-4">
                            <h3 className="font-semibold text-white/70 mb-1">{t.tags}</h3>
                    ...
                    {item.sellerName && (
                        <div className="mt-4 pt-4 border-t border-white/10">
                            <h3 className="font-semibold text-white/70 mb-2">{t.seller_info}</h3>
                ...
                <div className="p-4 border-t border-white/10 flex gap-4">
                    <button onClick={() => onFindSimilar(item)} className="flex-1 bg-white/10 text-white font-semibold py-2 rounded-lg hover:bg-white/20">✨ {t.find_similar}</button>
