...
const DailyBuzzSummaryCard: React.FC = () => {
    ...
    const fetchSummary = useCallback(async () => {
        ...
        try {
            ...
            const prompt = `
              Analyze recent social media buzz about Alexandria, Egypt.
              Generate a short, engaging summary (2-3 sentences) about the general sentiment and most discussed topics of the day.
    ...
    return (
        <div className="bg-base-dark-200/80 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-white/10 mb-4">
            <h3 className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary mb-2">{t.daily_buzz_summary}</h3>
            {isCardLoading ? (
                ...
                <p className="text-base-content-dark">{translatedData?.summary}</p>
            )}
        </div>
    );
};
